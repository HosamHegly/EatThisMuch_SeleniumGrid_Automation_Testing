...
class CardsEndpoints:
    ...
    def get_cards_by_class(self, class_name, body=None):
        return self.my_api.api_get_request(f"{self.url}/classes/{class_name}", body=body)

    def get_cards_by_race(self, race, body=None):
        return self.my_api.api_get_request(f"{self.url}/races/{race}", body=body)

    def get_cards_by_set(self, set_name, body=None):  # Fixed parameter name from 'class_name' to 'set_name'
        return self.my_api.api_get_request(f"{self.url}/sets/{set_name}", body=body)

    def get_cards_by_quality(self, quality, body=None):
        return self.my_api.api_get_request(f"{self.url}/qualities/{quality}", body=body)

    def get_cards_by_faction(self, faction, body=None):
        return self.my_api.api_get_request(f"{self.url}/factions/{faction}", body=body)

    def get_cards_by_type(self, type, body=None):
        return self.my_api.api_get_request(f"{self.url}/types/{type}", body=body)
